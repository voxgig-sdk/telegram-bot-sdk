-- TelegramBot SDK RepostStory entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RepostStoryEntity = {}
RepostStoryEntity.__index = RepostStoryEntity


function RepostStoryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RepostStoryEntity)
  self._name = "repost_story"
  self._client = client
...
function RepostStoryEntity:get_name()
  return self._name
end


function RepostStoryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RepostStoryEntity.new(self._client, opts)
end
...
function RepostStoryEntity:mark_deleted()
  self._deleted = true
end


function RepostStoryEntity:deleted()
  return true == self._deleted
end


function RepostStoryEntity:data_set(args)
  if args ~= nil then
...
function RepostStoryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RepostStoryEntity:match_set(args)
  if args ~= nil then
...
function RepostStoryEntity:match_get()
...
function RepostStoryEntity:stream(action, args, callopts)
...
function RepostStoryEntity:_run_op(ctx, post_done)
  local utility = self._utility
