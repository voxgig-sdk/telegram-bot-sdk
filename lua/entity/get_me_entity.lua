...
local GetMeEntity = {}
GetMeEntity.__index = GetMeEntity


function GetMeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetMeEntity)
  self._name = "get_me"
  self._client = client
...
function GetMeEntity:get_name()
  return self._name
end


function GetMeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetMeEntity.new(self._client, opts)
end
...
function GetMeEntity:mark_deleted()
  self._deleted = true
end


function GetMeEntity:deleted()
  return true == self._deleted
end


function GetMeEntity:data_set(args)
  if args ~= nil then
...
function GetMeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetMeEntity:match_set(args)
  if args ~= nil then
...
function GetMeEntity:match_get()
...
function GetMeEntity:stream(action, args, callopts)
...
function GetMeEntity:_run_op(ctx, post_done)
  local utility = self._utility
