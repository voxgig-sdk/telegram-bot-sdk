-- TelegramBot SDK SendMessageDraft entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local SendMessageDraftEntity = {}
SendMessageDraftEntity.__index = SendMessageDraftEntity


function SendMessageDraftEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SendMessageDraftEntity)
  self._name = "send_message_draft"
  self._client = client
...
function SendMessageDraftEntity:get_name()
  return self._name
end


function SendMessageDraftEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SendMessageDraftEntity.new(self._client, opts)
end
...
function SendMessageDraftEntity:mark_deleted()
  self._deleted = true
end


function SendMessageDraftEntity:deleted()
  return true == self._deleted
end


function SendMessageDraftEntity:data_set(args)
  if args ~= nil then
...
function SendMessageDraftEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SendMessageDraftEntity:match_set(args)
  if args ~= nil then
...
function SendMessageDraftEntity:match_get()
...
function SendMessageDraftEntity:stream(action, args, callopts)
...
---@param reqdata SendMessageDraftCreateData
---@param ctrl? table
---@return SendMessageDraft
---@return string? err
function SendMessageDraftEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function SendMessageDraftEntity:_run_op(ctx, post_done)
  local utility = self._utility
