...
local GetUserGiftEntity = {}
GetUserGiftEntity.__index = GetUserGiftEntity


function GetUserGiftEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetUserGiftEntity)
  self._name = "get_user_gift"
  self._client = client
...
function GetUserGiftEntity:get_name()
  return self._name
end


function GetUserGiftEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetUserGiftEntity.new(self._client, opts)
end
...
function GetUserGiftEntity:mark_deleted()
  self._deleted = true
end


function GetUserGiftEntity:deleted()
  return true == self._deleted
end


function GetUserGiftEntity:data_set(args)
  if args ~= nil then
...
function GetUserGiftEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetUserGiftEntity:match_set(args)
  if args ~= nil then
...
function GetUserGiftEntity:match_get()
...
function GetUserGiftEntity:stream(action, args, callopts)
...
function GetUserGiftEntity:_run_op(ctx, post_done)
  local utility = self._utility
