...
local GetChatGiftEntity = {}
GetChatGiftEntity.__index = GetChatGiftEntity


function GetChatGiftEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetChatGiftEntity)
  self._name = "get_chat_gift"
  self._client = client
...
function GetChatGiftEntity:get_name()
  return self._name
end


function GetChatGiftEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetChatGiftEntity.new(self._client, opts)
end
...
function GetChatGiftEntity:mark_deleted()
  self._deleted = true
end


function GetChatGiftEntity:deleted()
  return true == self._deleted
end


function GetChatGiftEntity:data_set(args)
  if args ~= nil then
...
function GetChatGiftEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetChatGiftEntity:match_set(args)
  if args ~= nil then
...
function GetChatGiftEntity:match_get()
...
function GetChatGiftEntity:stream(action, args, callopts)
...
function GetChatGiftEntity:_run_op(ctx, post_done)
  local utility = self._utility
