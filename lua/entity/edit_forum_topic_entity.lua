...
local EditForumTopicEntity = {}
EditForumTopicEntity.__index = EditForumTopicEntity


function EditForumTopicEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EditForumTopicEntity)
  self._name = "edit_forum_topic"
  self._client = client
...
function EditForumTopicEntity:get_name()
  return self._name
end


function EditForumTopicEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EditForumTopicEntity.new(self._client, opts)
end
...
function EditForumTopicEntity:mark_deleted()
  self._deleted = true
end


function EditForumTopicEntity:deleted()
  return true == self._deleted
end


function EditForumTopicEntity:data_set(args)
  if args ~= nil then
...
function EditForumTopicEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EditForumTopicEntity:match_set(args)
  if args ~= nil then
...
function EditForumTopicEntity:match_get()
...
function EditForumTopicEntity:stream(action, args, callopts)
...
function EditForumTopicEntity:_run_op(ctx, post_done)
  local utility = self._utility
