-- TelegramBot SDK PromoteChatMember entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PromoteChatMemberEntity = {}
PromoteChatMemberEntity.__index = PromoteChatMemberEntity


function PromoteChatMemberEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PromoteChatMemberEntity)
  self._name = "promote_chat_member"
  self._client = client
...
function PromoteChatMemberEntity:get_name()
  return self._name
end


function PromoteChatMemberEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PromoteChatMemberEntity.new(self._client, opts)
end
...
function PromoteChatMemberEntity:mark_deleted()
  self._deleted = true
end


function PromoteChatMemberEntity:deleted()
  return true == self._deleted
end


function PromoteChatMemberEntity:data_set(args)
  if args ~= nil then
...
function PromoteChatMemberEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PromoteChatMemberEntity:match_set(args)
  if args ~= nil then
...
function PromoteChatMemberEntity:match_get()
...
function PromoteChatMemberEntity:stream(action, args, callopts)
...
function PromoteChatMemberEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PromoteChatMemberEntity
