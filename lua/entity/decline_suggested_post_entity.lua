-- TelegramBot SDK DeclineSuggestedPost entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DeclineSuggestedPostEntity = {}
DeclineSuggestedPostEntity.__index = DeclineSuggestedPostEntity


function DeclineSuggestedPostEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DeclineSuggestedPostEntity)
  self._name = "decline_suggested_post"
  self._client = client
...
function DeclineSuggestedPostEntity:get_name()
  return self._name
end


function DeclineSuggestedPostEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DeclineSuggestedPostEntity.new(self._client, opts)
end
...
function DeclineSuggestedPostEntity:mark_deleted()
  self._deleted = true
end


function DeclineSuggestedPostEntity:deleted()
  return true == self._deleted
end


function DeclineSuggestedPostEntity:data_set(args)
  if args ~= nil then
...
function DeclineSuggestedPostEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DeclineSuggestedPostEntity:match_set(args)
  if args ~= nil then
...
function DeclineSuggestedPostEntity:match_get()
...
function DeclineSuggestedPostEntity:stream(action, args, callopts)
...
---@param reqdata DeclineSuggestedPostCreateData
---@param ctrl? table
---@return DeclineSuggestedPost
---@return string? err
function DeclineSuggestedPostEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function DeclineSuggestedPostEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return DeclineSuggestedPostEntity
