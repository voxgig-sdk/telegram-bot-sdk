-- TelegramBot SDK ForumTopic entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ForumTopicEntity = {}
ForumTopicEntity.__index = ForumTopicEntity


function ForumTopicEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ForumTopicEntity)
  self._name = "forum_topic"
  self._client = client
...
function ForumTopicEntity:get_name()
  return self._name
end


function ForumTopicEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ForumTopicEntity.new(self._client, opts)
end
...
function ForumTopicEntity:mark_deleted()
  self._deleted = true
end


function ForumTopicEntity:deleted()
  return true == self._deleted
end


function ForumTopicEntity:data_set(args)
  if args ~= nil then
...
function ForumTopicEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ForumTopicEntity:match_set(args)
  if args ~= nil then
...
function ForumTopicEntity:match_get()
...
function ForumTopicEntity:stream(action, args, callopts)
...
---@param reqdata ForumTopicCreateData
---@param ctrl? table
---@return ForumTopic
---@return string? err
function ForumTopicEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ForumTopicEntity:_run_op(ctx, post_done)
  local utility = self._utility
