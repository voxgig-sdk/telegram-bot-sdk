-- TelegramBot SDK MessageId entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local MessageIdEntity = {}
MessageIdEntity.__index = MessageIdEntity


function MessageIdEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MessageIdEntity)
...
function MessageIdEntity:get_name()
  return self._name
end


function MessageIdEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MessageIdEntity.new(self._client, opts)
end
...
function MessageIdEntity:mark_deleted()
  self._deleted = true
end


function MessageIdEntity:deleted()
  return true == self._deleted
end


function MessageIdEntity:data_set(args)
  if args ~= nil then
...
function MessageIdEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MessageIdEntity:match_set(args)
  if args ~= nil then
...
function MessageIdEntity:match_get()
...
function MessageIdEntity:stream(action, args, callopts)
...
---@param reqdata MessageIdCreateData
---@param ctrl? table
---@return MessageId
---@return string? err
function MessageIdEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function MessageIdEntity:_run_op(ctx, post_done)
  local utility = self._utility
