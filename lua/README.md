...
The Lua SDK for the TelegramBot API. Provides an entity-oriented interface using Lua conventions.
...
```lua
-- Create
local created, _ = client:ApproveSuggestedPost(nil):create({ name = "Example" }, nil)
...
local result, err = client:TelegramBot(nil):load(
  { id = "test01" }, nil
)
