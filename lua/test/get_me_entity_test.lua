...
describe("GetMeEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:GetMe(nil)
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TELEGRAM_BOT_TEST_GET_ME_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local get_me_ref01_ent = client:GetMe(nil)
    local get_me_ref01_data = helpers.to_map(vs.getprop(
      vs.getpath(setup.data, "new.get_me"), "get_me_ref01"))

    local get_me_ref01_data_result, err = get_me_ref01_ent:create(get_me_ref01_data, nil)
    assert.is_nil(err)
...
function get_me_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/get_me/GetMeTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read get_me test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("TELEGRAM_BOT_TEST_GET_ME_ENTID")
