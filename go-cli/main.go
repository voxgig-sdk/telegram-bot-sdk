// Command TelegramBot-cli is a CLI + REPL for the TelegramBot SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TELEGRAM_BOT_APIKEY (api key), TELEGRAM_BOT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TELEGRAM_BOT_APIKEY carries the API key and
	// TELEGRAM_BOT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TELEGRAM_BOT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TELEGRAM_BOT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTelegramBotSDK(opts)
