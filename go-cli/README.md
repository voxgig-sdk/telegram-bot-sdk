...
AQL-driven command-line client **and** interactive REPL for the TelegramBot
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/telegram-bot-cli)
make build
...
TELEGRAM_BOT_BASE=https://api.example.com ./telegram-bot-cli :help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/telegram-bot-cli
   ```
...
Run with no arguments to open a REPL (prompt `telegram-bot>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `approve_suggested_post`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/telegram-bot-cli`. |
...
(`./telegram-bot-cli <expr>`) and interactively (the REPL), and expressions compose the
