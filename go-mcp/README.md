...
[MCP](https://modelcontextprotocol.io) server exposing the TelegramBot SDK as
two agent tools — `telegram-bot_list` and `telegram-bot_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/telegram-bot-mcp)
make build

# 2. Provide credentials via the environment
export TELEGRAM_BOT_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user telegram-bot \
  -- /absolute/path/to/telegram-bot-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./telegram-bot-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// telegram-bot_list: first page of records
{ "entity": "update" }
{ "entity": "update", "query": { } }

// telegram-bot_load: one record by id
{ "entity": "get_me", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/telegram-bot-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user telegram-bot \
     -- "$PWD"/dist/*/telegram-bot-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `telegram-bot_list` and `telegram-bot_load` tools now appear
   in new sessions. Ask the agent to *"list update using telegram-bot"*
   and it calls `telegram-bot_list` with `{"entity":"update"}`.
...
```sh
export TELEGRAM_BOT_APIKEY=sk_live_xxx            # API key
export TELEGRAM_BOT_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `telegram-bot_list` tool
...
### Call the `telegram-bot_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `telegram-bot_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `telegram-bot_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TELEGRAM_BOT_APIKEY` | API key sent with every request. |
| `TELEGRAM_BOT_BASE` | Optional override of the API base URL. |
...
```sh
./telegram-bot-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"telegram-bot_load","arguments":{"entity":"get_me","query":{"id":1}}}}'
```
