...
func TestUnpinAllForumTopicMessageEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		unpinAllForumTopicMessageRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "unpin_all_forum_topic_message"}), "unpin_all_forum_topic_message_ref01"))
		...
		unpinAllForumTopicMessageRef01Data = core.ToMapAny(entityData(unpinAllForumTopicMessageRef01DataResult))
...
func unpin_all_forum_topic_messageBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TELEGRAM_BOT_TEST_UNPIN_ALL_FORUM_TOPIC_MESSAGE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TELEGRAM_BOT_TEST_UNPIN_ALL_FORUM_TOPIC_MESSAGE_ENTID"])
