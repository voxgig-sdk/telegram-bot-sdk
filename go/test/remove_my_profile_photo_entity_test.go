...
func TestRemoveMyProfilePhotoEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set TELEGRAM_BOT_TEST_REMOVE_MY_PROFILE_PHOTO_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		removeMyProfilePhotoRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "remove_my_profile_photo"}), "remove_my_profile_photo_ref01"))

		removeMyProfilePhotoRef01DataResult, err := removeMyProfilePhotoRef01Ent.Create(removeMyProfilePhotoRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		removeMyProfilePhotoRef01Data = core.ToMapAny(entityData(removeMyProfilePhotoRef01DataResult))
...
func remove_my_profile_photoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TELEGRAM_BOT_TEST_REMOVE_MY_PROFILE_PHOTO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TELEGRAM_BOT_TEST_REMOVE_MY_PROFILE_PHOTO_ENTID"])
