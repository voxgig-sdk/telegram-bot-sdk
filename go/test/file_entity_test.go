...
func TestFileEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set TELEGRAM_BOT_TEST_FILE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func fileBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TELEGRAM_BOT_TEST_FILE_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"TELEGRAM_BOT_TEST_FILE_ENTID": idmap,
		"TELEGRAM_BOT_TEST_LIVE":      "FALSE",
		"TELEGRAM_BOT_TEST_EXPLAIN":   "FALSE",
		"TELEGRAM_BOT_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["TELEGRAM_BOT_TEST_FILE_ENTID"])
