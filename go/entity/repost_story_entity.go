...
func NewRepostStoryEntity(client *core.TelegramBotSDK, entopts map[string]any) *RepostStoryEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *RepostStoryEntity) GetName() string { return e.name }
...
func (e *RepostStoryEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *RepostStoryEntity) Deleted() bool {
	return e.deleted
}


func (e *RepostStoryEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewRepostStoryEntity(e.client, opts)
}

func (e *RepostStoryEntity) Data(args ...any) any {
...
func (e *RepostStoryEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an RepostStory; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *RepostStoryEntity) DataTyped(data ...RepostStory) RepostStory {
	if len(data) > 0 {
		return typedFrom[RepostStory](e.Data(asMap(data[0])))
	}
	return typedFrom[RepostStory](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through RepostStory (all fields
// optional at the wire level).
func (e *RepostStoryEntity) MatchTyped(match ...RepostStory) RepostStory {
	if len(match) > 0 {
		return typedFrom[RepostStory](e.Match(asMap(match[0])))
	}
	return typedFrom[RepostStory](e.Match())
}
...
func (e *RepostStoryEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *RepostStoryEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}


func (e *RepostStoryEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}



func (e *RepostStoryEntity) Create(reqdata map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// RepostStoryCreateData and returns an RepostStory. It delegates to the untyped
// Create (identical runtime) and converts at the typed boundary.
func (e *RepostStoryEntity) CreateTyped(reqdata RepostStoryCreateData, ctrl map[string]any) (RepostStory, error) {
	res, err := e.Create(asMap(reqdata), ctrl)
	if err != nil {
		return RepostStory{}, err
	}
	return typedFrom[RepostStory](res), nil
}



func (e *RepostStoryEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *RepostStoryEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *RepostStoryEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
