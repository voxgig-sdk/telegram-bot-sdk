...
class TestGetMeDirect:
    ...
    def test_should_direct_load_get_me(self):
        setup = _get_me_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-get_me", "live" if setup["live"] else "unit")
        if _skip:
...
def _get_me_direct_setup(mockres):
