    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TELEGRAM_BOT_TEST_GET_ME_ENTID JSON to run live")
        client = setup["client"]
        ...
        get_me_ref01_data = helpers.to_map(runner.entity_data(get_me_ref01_ent.create(get_me_ref01_data, None)))
...
def _get_me_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("TELEGRAM_BOT_TEST_GET_ME_ENTID"))
