...
class TestForumTopicEntity:

    def test_should_create_instance(self):
        testsdk = TelegramBotSDK.test(None, None)
        ent = testsdk.ForumTopic(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _forum_topic_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "forum_topic." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TELEGRAM_BOT_TEST_FORUM_TOPIC_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        forum_topic_ref01_ent = client.ForumTopic(None)
        forum_topic_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.forum_topic"), "forum_topic_ref01"))

        forum_topic_ref01_data = helpers.to_map(runner.entity_data(forum_topic_ref01_ent.create(forum_topic_ref01_data, None)))
        assert forum_topic_ref01_data is not None



def _forum_topic_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/forum_topic/ForumTopicTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "TELEGRAM_BOT_TEST_FORUM_TOPIC_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("TELEGRAM_BOT_TEST_FORUM_TOPIC_ENTID"))
