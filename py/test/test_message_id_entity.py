    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TELEGRAM_BOT_TEST_MESSAGE_ID_ENTID JSON to run live")
        client = setup["client"]
        ...
        message_id_ref01_data = helpers.to_map(runner.entity_data(message_id_ref01_ent.create(message_id_ref01_data, None)))
...
def _message_id_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "TELEGRAM_BOT_TEST_MESSAGE_ID_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("TELEGRAM_BOT_TEST_MESSAGE_ID_ENTID"))
