...
class TelegramBotTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TelegramBotTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TelegramBotTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TelegramBotTestRunner.getenv("TELEGRAM_BOT_TEST_LIVE")
        override = TelegramBotTestRunner.getenv("TELEGRAM_BOT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TelegramBotTestRunner.getenv(key)
        ...
        explain = TelegramBotTestRunner.getenv("TELEGRAM_BOT_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if TelegramBotTestRunner._test_control is not None:
            return TelegramBotTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TelegramBotTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TelegramBotTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TelegramBotTestRunner.load_test_control()
...
def load_env_local():
    TelegramBotTestRunner.load_env_local()


def env_override(m):
    return TelegramBotTestRunner.env_override(m)


def entity_data(v):
    return TelegramBotTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TelegramBotTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TelegramBotTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TelegramBotTestRunner.load_test_control()


def live_client_options():
    return TelegramBotTestRunner.live_client_options()


def live_delay_ms():
    return TelegramBotTestRunner.live_delay_ms()
