...
class ApproveSuggestedPostEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TELEGRAMBOT_TEST_APPROVE_SUGGESTED_POST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function approve_suggested_post_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("TELEGRAMBOT_TEST_APPROVE_SUGGESTED_POST_ENTID");
