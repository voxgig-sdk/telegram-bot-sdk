...
class MessageEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TELEGRAM_BOT_TEST_MESSAGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function message_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("TELEGRAM_BOT_TEST_MESSAGE_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["TELEGRAM_BOT_TEST_MESSAGE_ENTID"]);
