...
class RepostStoryEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TELEGRAM_BOT_TEST_REPOST_STORY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $repost_story_ref01_data = Helpers::to_map(is_object($repost_story_ref01_data_result) && method_exists($repost_story_ref01_data_result, 'data_get') ? $repost_story_ref01_data_result->data_get() : $repost_story_ref01_data_result);
...
function repost_story_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("TELEGRAM_BOT_TEST_REPOST_STORY_ENTID");
