...
class GetMeEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = TelegramBotSDK::test(null, null);
        $ent = $testsdk->GetMe(null);
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TELEGRAM_BOT_TEST_GET_ME_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $get_me_ref01_ent = $client->GetMe(null);
...
function get_me_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/get_me/GetMeTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["get_me01", "get_me02", "get_me03"] as $k) {
    ...
    $entid_env_raw = getenv("TELEGRAM_BOT_TEST_GET_ME_ENTID");
