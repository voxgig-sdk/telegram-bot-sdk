...
The Ruby SDK for the TelegramBot API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
# Create
created, _ = client.ApproveSuggestedPost(nil).create({ "name" => "Example" }, nil)
...
result, err = client.TelegramBot(nil).load(
  { "id" => "test01" }, nil
)
