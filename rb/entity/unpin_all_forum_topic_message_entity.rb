...
class UnpinAllForumTopicMessageEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "unpin_all_forum_topic_message"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    UnpinAllForumTopicMessageEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TelegramBotHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [UnpinAllForumTopicMessage, Hash] the current UnpinAllForumTopicMessage data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TelegramBotHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of UnpinAllForumTopicMessage fields)
  def match_get
  ...
  #
  # @param reqdata [UnpinAllForumTopicMessageCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [UnpinAllForumTopicMessage, Hash] the created UnpinAllForumTopicMessage; raises TelegramBotError on failure
