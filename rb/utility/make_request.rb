...
module TelegramBotUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TelegramBotResponse.new({})
    ...
    if fetch_err
      ...
      response = TelegramBotResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TelegramBotResponse.new(fetched)
    else
