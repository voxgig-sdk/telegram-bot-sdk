...
class GetChatGiftEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TELEGRAM_BOT_TEST_GET_CHAT_GIFT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    get_chat_gift_ref01_data = Helpers.to_map(get_chat_gift_ref01_data_result.respond_to?(:data_get) ? get_chat_gift_ref01_data_result.data_get : get_chat_gift_ref01_data_result)
...
def get_chat_gift_basic_setup(extra)
  ...
  entid_env_raw = ENV["TELEGRAM_BOT_TEST_GET_CHAT_GIFT_ENTID"]
