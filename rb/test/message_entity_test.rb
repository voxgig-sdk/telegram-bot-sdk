...
class MessageEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TELEGRAMBOT_TEST_MESSAGE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    message_ref01_data_result, err = message_ref01_ent.create(message_ref01_data, nil)
    assert_nil err
...
def message_basic_setup(extra)
  ...
  entid_env_raw = ENV["TELEGRAMBOT_TEST_MESSAGE_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "TELEGRAMBOT_TEST_MESSAGE_ENTID" => idmap,
  ...
  idmap_resolved = Helpers.to_map(
    env["TELEGRAMBOT_TEST_MESSAGE_ENTID"])
