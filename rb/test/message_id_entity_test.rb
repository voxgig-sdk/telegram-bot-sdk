...
class MessageIdEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = message_id_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "message_id." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TELEGRAM_BOT_TEST_MESSAGE_ID_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    message_id_ref01_ent = client.MessageId(nil)
    ...
    message_id_ref01_data = Helpers.to_map(message_id_ref01_data_result.respond_to?(:data_get) ? message_id_ref01_data_result.data_get : message_id_ref01_data_result)
    assert !message_id_ref01_data.nil?

  end
end

def message_id_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "message_id", "MessageIdTestData.json")
  ...
  entid_env_raw = ENV["TELEGRAM_BOT_TEST_MESSAGE_ID_ENTID"]
