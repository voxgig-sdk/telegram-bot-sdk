...
class SetMyProfilePhotoEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TELEGRAM_BOT_TEST_SET_MY_PROFILE_PHOTO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    set_my_profile_photo_ref01_data = Helpers.to_map(set_my_profile_photo_ref01_data_result.respond_to?(:data_get) ? set_my_profile_photo_ref01_data_result.data_get : set_my_profile_photo_ref01_data_result)
...
def set_my_profile_photo_basic_setup(extra)
  ...
  entid_env_raw = ENV["TELEGRAM_BOT_TEST_SET_MY_PROFILE_PHOTO_ENTID"]
