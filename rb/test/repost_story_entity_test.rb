...
class RepostStoryEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TELEGRAM_BOT_TEST_REPOST_STORY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    repost_story_ref01_data = Helpers.to_map(repost_story_ref01_data_result.respond_to?(:data_get) ? repost_story_ref01_data_result.data_get : repost_story_ref01_data_result)
...
def repost_story_basic_setup(extra)
  ...
  entid_env_raw = ENV["TELEGRAM_BOT_TEST_REPOST_STORY_ENTID"]
