...
class PromoteChatMemberEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TELEGRAM_BOT_TEST_PROMOTE_CHAT_MEMBER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    promote_chat_member_ref01_data = Helpers.to_map(promote_chat_member_ref01_data_result.respond_to?(:data_get) ? promote_chat_member_ref01_data_result.data_get : promote_chat_member_ref01_data_result)
...
def promote_chat_member_basic_setup(extra)
  ...
  entid_env_raw = ENV["TELEGRAM_BOT_TEST_PROMOTE_CHAT_MEMBER_ENTID"]
