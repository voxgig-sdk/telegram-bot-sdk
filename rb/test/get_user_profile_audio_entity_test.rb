...
class GetUserProfileAudioEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TELEGRAM_BOT_TEST_GET_USER_PROFILE_AUDIO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    get_user_profile_audio_ref01_data_result = get_user_profile_audio_ref01_ent.create(get_user_profile_audio_ref01_data, nil)
    get_user_profile_audio_ref01_data = Helpers.to_map(get_user_profile_audio_ref01_data_result.respond_to?(:data_get) ? get_user_profile_audio_ref01_data_result.data_get : get_user_profile_audio_ref01_data_result)
...
def get_user_profile_audio_basic_setup(extra)
  ...
  entid_env_raw = ENV["TELEGRAM_BOT_TEST_GET_USER_PROFILE_AUDIO_ENTID"]
