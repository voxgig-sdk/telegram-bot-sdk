...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.TELEGRAMBOT_TEST_LIVE ||
    'TRUE' === process.env.TELEGRAMBOT_TEST_OVERRIDE
  ) {
  ...
  m.TELEGRAMBOT_TEST_EXPLAIN = process.env.TELEGRAMBOT_TEST_EXPLAIN || m.TELEGRAMBOT_TEST_EXPLAIN
