...
class FtCtrl extends TelegramBotControl
{
...
class FtClient
{
    ...
    private TelegramBotUtility $utility;

    public function __construct(TelegramBotUtility $utility, array $options)
    {
    ...
    public function get_utility(): TelegramBotUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TelegramBotContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TelegramBotContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TelegramBotContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TelegramBotConfig::make_feature($name);
    ...
    private static function buildUrl(TelegramBotSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TelegramBotError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TelegramBotResponse($fetched);
        }
    ...
    private function fail(TelegramBotContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TelegramBotContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
